...
def mission():
    ...
    model = PPO("MlpPolicy", t, tensorboard_log="/home/javilinos/PPO", verbose=1, n_steps=256, batch_size=64, gae_lambda=0.9, gamma=0.99, n_epochs=10, max_grad_norm=0.3, ent_coef=0.001, vf_coef=0.5, clip_range=0.2, learning_rate=3e-05, use_sde=True, policy_kwargs=dict(log_std_init=-1,
                        ortho_init=False,
                        activation_fn=th.nn.ReLU,
                        net_arch=[dict(pi=[256, 256], vf=[256, 256])]
                        ))
    print("Starting mission...")
    model.learn(total_timesteps=700000, callback=cb)
    obs = t.reset()
