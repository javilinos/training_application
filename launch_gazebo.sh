...
#SIMULATE WITH CAMERA
	(cd $(rospack find px4) ; git submodule update --init --recursive ; DONT_RUN=1 make DCMAKE_BUILD_TYPE=Release px4_sitl gazebo)
...
roslaunch configs/gazebo/launch/multi_uav_mavros_sitl_sdf.launch world:=${AEROSTACK_PROJECT}/configs/gazebo/worlds/empty.world vehicle:=iris sdf1:=${AEROSTACK_PROJECT}/configs/gazebo/models/drone1/iris/iris.sdf sdf2:=${AEROSTACK_PROJECT}/configs/gazebo/models/drone2/iris/iris.sdf sdf3:=${AEROSTACK_PROJECT}/configs/gazebo/models/drone3/iris/iris.sdf sdf4:=${AEROSTACK_PROJECT}/configs/gazebo/models/drone4/iris/iris.sdf respawn_gazebo:=true
